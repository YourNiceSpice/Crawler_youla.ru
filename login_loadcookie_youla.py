...
def text_generator():
    ...
    second = ['Перестал заниматься кроссовками,поэтому распродаю остатки товара.','Закрываю свой бизнес,ищу покупателя на свой ассортимент.','В связи с закрытием своего магазина,продаю свой товар оптом','Возможно вас заинтересует мои кроссовки,закрываю магазин распродаю оптом,ниже цены всех поставщиков.',,]
    third = [' Заказывал модели у Монтана-опт,Кроссовки-опт-Мск.','Моим поставщиком были Монтана-опт.','В основном заказывал в Монтана-опт из Москвы.']
    fourth = ['Размеры остались разные,продам от 4х пар любой модели.Цены от 900р-до 1300.','Продам от четырех пар любой модели,размеры нужно уточнять,остались не все.']
    fifth = ['Весь ассортимент в профиле,можете выбрать.После договоримся о цене.','Все модели посмотрите на моем акаунте.Выберете то что нужно,по цене соориентирую.', 'По стоимостии соориетирую.Если нужно оставлю свои контакты для связи' ]
    
    s = random.choice(first) +'\n'+ random.choice(second) +'\n'+ random.choice(third) +'\n'+ random.choice(fourth) +'\n'+ random.choice(fifth)
    return s    
...
for i in item_list:
    ...
    if seller_or_not.seller_or_not(text):
        ...
        actions.move_to_element(element).send_keys(text_generator(),Keys.RETURN).perform()
